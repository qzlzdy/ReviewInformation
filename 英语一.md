...
- 一幅图一个主体两个动作` in the cartoon, sb(主语)[定], doing sth(次要动作)[定|状], is doing sth(主要动作)[定|状] `
- 一幅图两个主体两个动作` in the cartoon, sb1[定] is doing sth1[定|状]; ({and}at the same time/{but}on the contrary) sb2[定] is doing sth2[定|状] `
- 两幅图` in the former picture, sb1[定] is doing sth1[定|状]; ({and}at the same time/{but}on the contrary) in the latter one, sb2[定] is doing sth2[定|状] `

#### 第三句

引出文章中心的过渡语

` we can deduce from this picture that the auther is trying to attract our attention to sth, which... `
...
` sth, needless to say, is supposed to argued extraordinarily indispensable by an increasing number of children and adults`
...
1. `Sth is a {defination} which means sb do sth1, sb do sth2 and sb do sth3 when ...`
2. `There exist too plentiful cases like this in the routine of our life to list. However, the most impressive one is (the following / about myself). {example}. So the story vividly tells us a truth that ...`
...
>example:
>
>I ... because ... . At that moment, ... . Then I ... .
...
`to be short, it is sth that if +`虚拟语气`+ we would do`
...
他人：`if senior citizens who ... could do sth(kids), ...`虚拟语气

#### 第四句

呼吁

`shouldn't human beings place great emphasis on the current situation of sth?`
...
文中未给出：`I am a undergraduate from the department of art design in a/the university`

#### 写作目的

`I am writing the letter, to be frank, in order to {purpose}`
...
`in addition, it seems beyond depute that ... [therefore/at the same time/on the contrary]`
...
练习方式`if you intended to join us, please email us at xxx@xxx`
