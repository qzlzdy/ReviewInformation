...
$ f(x)\ge0,\exist p\ge1使得\lim\limits_{x\to a^+}(x-a)^pf(x)=\begin{cases}A>0\\+\infin\end{cases}\Rarr\int_a^bf(x)dx发散 $
...
|                           未知参数                           |                     $ 1-\alpha $置信区间                     |
| :----------------------------------------------------------: | :----------------------------------------------------------: |
|                          $ \sigma $                          | $ \left(\frac{(n-1)S^2}{\chi^2_{\frac\alpha2}(n-1)},\frac{(n-1S^2)}{\chi^2_{1-\frac\alpha2}(n-1)}\right) $ |
...
又$ \int_0^{+\infin}f^2(t)dt $收敛，设$ \int_0^{+\infin}f^2(t)dt=A $，则
...
令$ F(x)=e^{g(x)}[f(x)-f(\eta)] $，由罗尔定理

$ \exist\xi\in(c,\eta) $，使得$ F'(\xi)=0 $

又$ F'(x)=e^{g(x)}g'(x)[f(x)-f(\eta)]+e^{g(x)}f'(x) $

则$ f'(\xi)=g'(\xi)[f(\eta)-f(\xi)] $